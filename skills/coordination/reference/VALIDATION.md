...
### 8. Demonstrated-impact bar for High/Critical

A proposed High/Critical must demonstrate one of the four **impact rungs** — **shell or box/machine access by any path — reverse shell, webshell, an RCE session on the box, or a single-shot `ssh -o BatchMode=yes` login with any discovered key/password — whichever the primitive supports**, **admin/service-account access**, a **database exfiltration proof** (auth proof + bounded enumeration), or **financial harm — on crypto targets the rung is a **fund PoC**: signing authority over at-risk funds demonstrated with a signed message from the compromised key (broadcast NOTHING, transfer NOTHING — the valid signature is the proof of control), plus the quantified funds-at-risk figure** — see [`severity-calibration.md`](severity-calibration.md) rule 6. Not demonstrated **and** not blocked by a transient/reversible condition → **cap at Medium**: recompute the vector's C/I/A on the demonstrated rung so Check 1 stays self-consistent (a bare High/Critical vector with a Medium label fails `cvss_lint`), set `needs_live_confirmation: true`, and carry the theoretical ceiling + cap rationale in the `calibration` field. Check 6 still governs the transient-blocker case — the bar bounds rungs never climbed, the floor bounds rungs blocked by incidental state. A finding that *did* climb to a demonstrated rung scores at that rung's real impact — the bar and the exploitation mandate are two halves of one rule: climb the rungs, and the severity is earned.
